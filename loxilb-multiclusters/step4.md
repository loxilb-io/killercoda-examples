...
cat > loxi-ccm-playground1.yaml <<EOF
---
apiVersion: v1
kind: ServiceAccount
metadata:
    ...
    spec:
      ...
      tolerations:
        ...
        - key: node-role.kubernetes.io/control-plane
          effect: NoSchedule
      nodeSelector:
        kubernetes.io/hostname: k8s-playground1-worker
EOF
cat > loxi-ccm-playground2.yaml <<EOF
---
apiVersion: v1
kind: ServiceAccount
metadata:
    ...
    spec:
      ...
      tolerations:
        ...
        - key: node-role.kubernetes.io/control-plane
          effect: NoSchedule
      nodeSelector:
        kubernetes.io/hostname: k8s-playground2-worker
EOF
```

Deploy CCM DaemonSet service on k8s-playgoround1 through following command:

```
kubectl create -f loxi-ccm-playground1.yaml --context kind-k8s-playground1
```

You can check CCM daemon status trough following command:

```
kubectl get pods -A --context kind-k8s-playground1 | grep loxi
...
Deploy CCM DaemonSet service on k8s-playgoround2 through following command:

```
kubectl create -f loxi-ccm-playground2.yaml --context kind-k8s-playground2
```

You can check CCM daemon status trough following command:

```
kubectl get pods -A --context kind-k8s-playground2 | grep loxi
kube-system          loxi-cloud-controller-manager-wzjqz                    1/1     Running   0          12m
```
